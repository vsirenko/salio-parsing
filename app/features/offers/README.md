...
| `POST /api/admin/sources/{source_id}/reread` | read a channel's listings again from their stored payloads, for one a reparse cannot reach; `brand_id` narrows it to one maker's |
